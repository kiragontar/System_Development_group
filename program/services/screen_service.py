...
class ScreenService:
    ...
    def check_screen_in_use(self, screen_id: str, datetime: datetime) -> bool:
        """Checks if a screen is in use at a given datetime."""
        screenings = self.screening_service.get_screenings_for_screen(screen_id)
        for screening in screenings:
            if screening.start_time is not None and screening.end_time is not None and screening.start_time <= datetime <= screening.end_time:
                return True
        return False