...
@pytest.fixture
def create_test_data(session, cinema):  #Creates test data for the screen, and film tables.
    screen = Screen.create_screen(screen_id="S1", cinema_id=cinema.cinema_id, capacity_upper=80, capacity_lower=30, capacity_vip=10)
...
def test_create_screening(screening_service, session, create_test_data): # Pytest automatically discovers and runs this test function. screening_service, session and create_test_data are fixtures that pytest injects into the function.
    ...
    assert screening.end_time == end_time
